// https://github.com/deep-esh7/hexify/blob/main/lib/hexify.dart
import 'package:flutter/material.dart';

/// Enumeration for gradient types to provide users with choices between linear or radial gradients.
enum HexifyGradientType { linearGradient, radialGradient }
...
dynamic Hexify({
  ...
  double?
  opacity, // Opacity can be specified as a percentage (1 to 100) or as a decimal (0.0 to 1.0).
  HexifyGradientType?
  gradientType, // Specify the type of gradient if using gradient colors.
  ...
  double?
  firstColorShade, // Optional shade factor for the first color, no default to ensure explicit use.
  ...
  AlignmentGeometry begin =
      Alignment.topCenter, // Start position for LinearGradient.
  AlignmentGeometry end =
      Alignment.bottomCenter, // End position for LinearGradient.
  List<double>?
  stops, // Stops for defining positions of color transitions within the gradient.
  TileMode?
  tileMode, // How the gradient should tile the plane beyond the gradient itself.
  ...
  if ((colorCode != null && gradientType != null) ||
      (colorCode != null &&
          (firstColor != null || secondColor != null || thirdColor != null)) ||
  ...
  if (firstColor != null) {
    gradientColors.add(
      _hexToColor(firstColor, opacityOfFirstColor, firstColorShade),
    );
  }
  if (secondColor != null) {
    gradientColors.add(
      _hexToColor(secondColor, opacityOfSecondColor, secondColorShade),
    );
  }
  if (thirdColor != null) {
    gradientColors.add(
      _hexToColor(thirdColor, opacityOfThirdColor, thirdColorShade),
    );
  }
...
Color _hexToColor(String hexCode, [double? opacity, double? shade]) {
  ...
  int alpha = hexCode.length == 8
      ? int.parse(hexCode.substring(0, 2), radix: 16)
      : 255;
  int colorInt = int.parse(
    hexCode.length == 8 ? hexCode.substring(2) : hexCode,
    radix: 16,
  );
...
Color _parseRGBO(String colorCode, [double? shade, double? opacity]) {
  final match = RegExp(
    r'^(\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})(?:,\s*(\d?\.?\d+))?$',
  ).firstMatch(colorCode);
  if (match == null ||
      int.parse(match.group(1)!) > 255 ||
